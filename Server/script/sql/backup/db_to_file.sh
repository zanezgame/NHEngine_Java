...
mysqldump -h127.0.0.1 -uroot -proot --default-character-set=utf8 --comments=TRUE --no-data=TRUE --add-drop-database=TRUE --add-drop-table=TRUE --single-transaction --databases wind_gm | sed 's/AUTO_INCREMENT=[0-9]*\s//g' > $data_dir/wind_gm.sql
