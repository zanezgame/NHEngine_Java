...
public class CG_AuthTokenReqHandler extends LogicHandler
{

	private static Logger logger = LoggerFactory.getLogger(CG_AuthTokenReqHandler.class);

	@Override
	public void handle(ChannelHandlerContext ctx, Message msg)
	{
		AuthTokenReq request = (AuthTokenReq)msg.protoBuf;
		logger.info("recv CG_AuthTokenReq, playerId = {}", msg.getPlayerId());
		...
		try
		{
			do
			{
				...
				else
				{
					...
					msg.setProtoBuf(queryTokenbuilder.build());
